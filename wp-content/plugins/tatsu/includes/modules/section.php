...
if (!function_exists('tatsu_section')) {
	function tatsu_section( $atts, $content ) {
		...
		if( isset( $bg_overlay ) && 1 == $bg_overlay ) {
			...
			$bg_overlay_markup .= '<div class="tatsu-overlay" style="background: '.$overlay_color.';"></div>';
		}
	    ...
	    $output .= '<div id="'.$section_id.'" class="tatsu-section '.$section_class.' tatsu-clearfix" style="'.$background.$border.$margin.'" data-title="'.$section_title.'" data-headerscheme="'.$full_screen_header_scheme.'">';
